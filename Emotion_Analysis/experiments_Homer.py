...
def optimization():
	optimized=BayesianOptimization(cross_validation, {'ngrams':(1,3), 'skipbigrams':(0,2), 'mini':(1,50), 'clusters':(2,5)})
	optimized.maximize(init_points=25, n_iter=10, kappa=2)
	print(optimized.res['max'])

def cross_validation(ngrams,skipbigrams, mini, clusters):
	params={}
	...
	params['clusters']=roundToNearestMultiple(clusters,1)
	print(params)
	for cv in range(0,10):
		...
		os.system(perl)
	mulanLoader.createHOMER(params['clusters'])
...
def train(process):
	...
	params['extra']=extraFeatures
	#----------------------------------------------LEMMA
	if(modelDictName=="NRC_Ortu_homer_lemma"):
		params['ngrams']=2
		params['skipbigrams']=2
		params['mini']=15
		params['clusters']=6
	elif(modelDictName=="NRC_OrtuNN_homer_lemma"):	#It wasn't optimized, but we need less clusters than labels
		params['ngrams']=2
		params['skipbigrams']=2
		params['mini']=15
		params['clusters']=5
	elif(modelDictName=="Sentic_OrtuNN_homer_lemma"):
		params['ngrams']=3
		params['skipbigrams']=0
		params['mini']=32
		params['clusters']=2
	elif(modelDictName=="NRC_SONN_homer_lemma"):
		params['ngrams']=1
		params['skipbigrams']=1
		params['mini']=25
		params['clusters']=4
	elif(modelDictName=="Sentic_SONN_homer_lemma"):
		params['ngrams']=1
		params['skipbigrams']=1
		params['mini']=15
		params['clusters']=3
	#----------------------------------------------NO LEMMA
	elif(modelDictName=="NRC_Ortu_homer_no_lemma"):
		params['ngrams']=2
		params['skipbigrams']=1
		params['mini']=10
		params['clusters']=4
	elif(modelDictName=="NRC_OrtuNN_homer_no_lemma"):	#It wasn't optimized, but we need less clusters than labels
		params['ngrams']=2
		params['skipbigrams']=1
		params['mini']=10
		params['clusters']=4
	elif(modelDictName=="Sentic_OrtuNN_homer_no_lemma"):
		params['ngrams']=3
		params['skipbigrams']=0
		params['mini']=1
		params['clusters']=2
	elif(modelDictName=="NRC_SONN_homer_no_lemma"):
		params['ngrams']=1
		params['skipbigrams']=0
		params['mini']=4
		params['clusters']=4
	elif(modelDictName=="Sentic_SONN_homer_no_lemma"):
		params['ngrams']=1
		params['skipbigrams']=0
		params['mini']=5
		params['clusters']=5
	else:
		print >> sys.stderr, "unknow parameters"
		sys.exit(1)

	if(process=="train_cv"):
		cross_validation(params['ngrams'],params['skipbigrams'], params['min'], params['clusters'])
	elif(process=="train"):
		params['pathMulan']=pathHomer
		params['pathInput']=pathInput
		params['nameTrain']=nameTrain
...
def test():
	...
	os.system(perl)
	name=modelDictPath + "/" + modelDictName + ".mulan"
	result=mulanLoader.predictWithEval("Homer", name)
	print(result)
	print("\n")
...
def predict():
	...
	mulanLoader.predict("Homer", name, outputFile)
	print("\n")
